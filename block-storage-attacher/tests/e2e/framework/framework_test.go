...
import (
	...
	registered "k8s.io/apimachinery/pkg/api/meta"
...
func NewFramework(baseName string, options FrameworkOptions, client clientset.Interface) *Framework {
	...
	if len(pluginName) == 0 {
		f.PluginNameShort = "ibmc-file"
	} else {
		f.PluginNameShort = strings.Split(pluginName, "/")[1]
	}
...
func (f *Framework) BeforeEach() {
	...
	if f.ClientSet == nil {
		...
		if TestContext.KubeAPIContentType != "" {
			Logf("TestContext.KubeAPIContentType:  %s", TestContext.KubeAPIContentType)
	...
	Logf("TestContext.Host:  %s", TestContext.Host)
	By("Building a namespace api object")
	namespace, err := f.CreateNamespace(f.BaseName, map[string]string{
		"e2e-framework": f.BaseName,
	})
	Expect(err).NotTo(HaveOccurred())
	By("Namespace api object created")
