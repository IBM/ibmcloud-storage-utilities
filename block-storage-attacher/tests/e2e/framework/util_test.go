...
const (
	...
	ClaimProvisionTimeout = 35 * time.Minute
...
func NewPersistentVolumeClaimDescriber(c clientset.Interface) *PersistentVolumeClaimDescriber {
	return &PersistentVolumeClaimDescriber{c}
}
...
func WaitForPersistentVolumeClaimPhase(phase v1.PersistentVolumeClaimPhase, c clientset.Interface, ns string, pvcName string, Poll, timeout time.Duration) error {
	...
	for start := time.Now(); time.Since(start) < timeout; time.Sleep(Poll) {
		...
		persistentVolumeClaimDescriber := NewPersistentVolumeClaimDescriber(c)
		pvcDescribeMsg = persistentVolumeClaimDescriber.PvcDescribe(ns, pvc)
		if err != nil {
	...
	return fmt.Errorf("PersistentVolumeClaim %s not in phase %s within %v for reason: %s", pvcName, phase, timeout, pvcDescribeMsg)
}
...
func CreateTestingNS(baseName string, c clientset.Interface, labels map[string]string) (*v1.Namespace, error) {
	...
	if err := wait.PollImmediate(Poll, 30*time.Second, func() (bool, error) {
		...
		if err != nil {
			...
			got, err = c.Core().Namespaces().Get("blockattacher-e2e-ns")
			if err != nil {
...
func deleteNS(c clientset.Interface, clientPool dynamic.ClientPool, namespace string, timeout time.Duration) error {
	...
	if remainingContent {
		...
		remainingPods, missingTimestamp, _ = countRemainingPods(c, namespace) //#nosec G104 test file
	}
...
func PodRunning(event watch.Event) (bool, error) {
	switch event.Type {
	case watch.Deleted:
		return false, apierrs.NewNotFound(metav1.GroupResource{Resource: "pods"}, "")
	}
...
func (d *PersistentVolumeClaimDescriber) PvcDescribe(namespace string, pvc *v1.PersistentVolumeClaim) string {

	events, _ := d.Core().Events(namespace).Search(pvc)

	pvcMessage := ""
	for _, e := range events.Items {
		pvcMessage = e.Message
		Logf("PersistenVolumeClaim message is %s", e.Message)
	}
