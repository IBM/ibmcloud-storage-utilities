...
function install_portworx_plugin {
	...
	check_pod_state "tiller-deploy"
        ...
        # INSTALL/UPGRADE HELM CHART
	helm_values_override="--set kvdb=etcd:$ETCD_SET1,clusterName=$(uuidgen),etcd.credentials=$ETCDCREDS,usedrivesAndPartitions=true,usefileSystemDrive=true,imageVersion=$PORTWORXVER,secretType=$IBMSECRETTYPE,drives=none,etcd.certPath=$CERTPATH,etcd.ca=$CAPATH"
