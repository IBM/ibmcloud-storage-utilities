...
func TestGetContextLoggerContext(t *testing.T) {
	...
	ctx := context.WithValue(context.Background(), RequestIDLabel, requestID)
...
func TestAddContextFields(t *testing.T) {
	...
	ctx = context.WithValue(ctx, TriggerKeyLabel, triggerKey)
	parentLogger, _ := GetZapLogger()
...
func TestCreateRequestIdField(t *testing.T) {
	...
	if field.Key != RequestIDLabel {
		t.Errorf("Expected key value to be: %s", RequestIDLabel)
	}
...
func TestCreateTriggerKeyField(t *testing.T) {
	triggerKey := "myTriggerKey"
	ctx := context.WithValue(context.Background(), TriggerKeyLabel, triggerKey)
	field := CreateZapTiggerKeyField(ctx)
	if field.Key != TriggerKeyLabel {
		t.Errorf("Expected key value to be: %s", TriggerKeyLabel)
	}
...
func TestCreatePodNameLogger(t *testing.T) {
	os.Setenv(PodNameEnvVar, "myPodName")
	logger, err := GetZapLogger()
	if err != nil {
		t.Errorf("Got error when creating global logger: %s", err.Error())
	}
...
func TestCreatePodNameKeyField(t *testing.T) {
	...
	os.Setenv(PodNameEnvVar, podNameValue)
...
func TestGenerateContextWithRequestID(t *testing.T) {
	...
	requestID, ok := ctx.Value(RequestIDLabel).(string)
	assert.True(t, ok)
